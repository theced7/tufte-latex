...
\usepackage{xspace}
\newcommand{\abbr}[2]{\mbox{#1.\,#2.}\xspace} % for short abbrevations (e.g. \abbr{Z}{n}) 

\usepackage{tcolorbox} % frames around stuff
\newtcolorbox{mybox}[3][]
{
  colframe = #2!25,
  colback  = #2!10,
...
\newenvironment{tablist}{\begin{itemize}[nosep]}{\end{itemize}} %no space before item (for tables)

% Added packages
\usepackage{siunitx} % to use units: \[v=\SI{2.3}{\metre\per\second}\]
\sisetup{
  locale = DE ,
  per-mode = symbol ,
  tophrase=\text{ bis },
